...
export class AuthService {
    ...
    async login(user: IUser, response: Response) {
        const { _id, name, email, age, address, role, permissions, company } = user;
        const payload = {
            sub: "token login",
            iss: "from server",
            _id,
    ...
    processNewToken = async (refreshToken: string, response: Response) => {
        try {
            ...
            if (user) {
                const { _id, name, email, age, address, role, company } = user;
                const payload = {
                    sub: "token refresh",
                    iss: "from server",
                    _id,
    ...
    getAccount = async (user: IUser) => {
        const foundUser = await this.usersService.findOne(user._id) as any;
        const temp = await this.rolesService.findOne(foundUser?.role?._id) as any;
        return {
            user: {
                _id: foundUser?._id,
